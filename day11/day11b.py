...
def expand_coords(coords, expansion_rate):
    """
    ...
    expansion_rate: int
        number of lines by which an empty position is replaced
    ...
    for empty_space in empty_list[::-1]:
        coords[coords > empty_space] += expansion_rate - 1
    return list(coords)
...
ycoords = expand_coords(ycoords, int(1e6))
xcoords = expand_coords(xcoords, int(1e6))
